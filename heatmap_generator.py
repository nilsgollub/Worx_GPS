...
import branca.element  # NEUER IMPORT für HTML Overlay
...
class HeatmapGenerator:
    ...
    def create_heatmap(self, data, html_file, draw_path, is_multi_session=False):
        ...
        # --- Logik für Kartentyp (QUALITÄT / WIFI) ---
        if visualize_quality_path or visualize_wifi_path:
            is_wifi = visualize_wifi_path
            logger.info(f"Erstelle gewichtete Karte für {'WiFi' if is_wifi else 'Qualität'} ({html_file}).")
            ...
            # Iteriere durch Sessions (wie bei normaler Karte)
            for idx, session_data in enumerate(reversed(sessions_to_draw)):
                ...
                show_layer = (idx == 0) # Nur die erste (neueste) Session standardmäßig zeigen
                ...
                if heat_data:
                    # Kombiniertes FeatureGroup für diese Session
                    layer_name = f"{'WiFi' if is_wifi else 'GPS'} {session_date_str}"
                    fg = folium.FeatureGroup(name=layer_name, show=show_layer)
                    
                    # Heatmap-Layer in die Gruppe
                    plugins.HeatMap(heat_data, radius=hm_radius, blur=hm_blur, 
                                    gradient=gradient, min_opacity=0.2).add_to(fg)
                    
                    # Optional: Dünner Pfad als Skelett
                    if draw_path and len(coords) > 1:
                        folium.PolyLine(coords, color='white', weight=1, opacity=0.4).add_to(fg)
            ...
            # Index fixen für Legende
            index_float = sorted([float(x) for x in index])
            min_v = index_float[0] - 5
            max_v = index_float[-1] + 5
            f_index = sorted(list(set([min_v] + index_float + [max_v])))
            ...
            if is_multi_session:
                for idx, session_data in enumerate(reversed(sessions_to_draw)):
                    ...
                    if session_heat_data:
                        ...
                        heatmap_layers.append(heatmap_feature_group)
                        if use_time_heatmap:
                            logger.warning(
            ...
            # --- Single-Session Fall (z.B. heatmap_aktuell) ---
                ...
                if session_data_single:
                    ...
                    # --- Heatmap erstellen (Normal oder Mit Zeit) ---
                    heatmap_added = False
                    if use_time_heatmap and start_time_ts and end_time_ts and single_points_full:
                        ...
                        try:
                            ...
                            while current_ts <= end_time_ts:
                                ...
                                current_ts += 1  # Schrittweite 1 Sekunde
                            for point in single_points_full:
                                try:
                                    ...
                                    # Finde den passenden Zeitindex (oder den nächsten davor)
                                    index_dt = max(dt for dt in time_index if dt <= point_dt)
                                    time_indexed_data[index_dt].append([lat, lon, 1.0])
                                except (ValueError, KeyError, TypeError, StopIteration):
                                    continue
                            hmt_data = []
                            accumulated_points = set()  # Verwende Set für eindeutige Punkte
                            for dt_index in time_index:
                                points_in_step = time_indexed_data.get(dt_index, [])
                                # Füge nur neue Punkte hinzu (als Tupel wegen Set)
                                accumulated_points.update(tuple(p) for p in points_in_step)
                                # Konvertiere zurück zu Liste von Listen für HMT
                                hmt_data.append([list(p) for p in accumulated_points])
                    ...
                    if not heatmap_added:
                        if single_heat_data:
                            ...
                            logger.debug(f"Creating standard Heatmap FeatureGroup '{heatmap_layer_name}'")
                            heatmap_feature_group_single = folium.FeatureGroup(name=heatmap_layer_name, show=show_layer)
                            plugins.HeatMap(single_heat_data,
                                            radius=heatmap_radius,
                                            blur=heatmap_blur
                                            ).add_to(heatmap_feature_group_single)
                            heatmap_feature_group_single.add_to(map_obj)
                            heatmap_layers.append(heatmap_feature_group_single)
                        else:
            ...
            # --- Bounds-Berechnung und Fallback ---
                ...
                try:
                    ...
                    if path_layers:
                        ...
                        for hm_layer in heatmap_layers:
                            ...
                            elif isinstance(hm_layer, folium.FeatureGroup):
        ...
        # --- NEU: Berechnung der Gesamtwerte für die *gesamte* Karte ---
        total_map_distance_m = 0.0
        total_map_duration_seconds = 0.0

        if sessions_to_draw:
            for session_data in sessions_to_draw:
                if not session_data or len(session_data) < 2:
                    continue
                
                # Distanz für diese Session aufsummieren
                for i in range(len(session_data) - 1):
                    try:
                        total_map_distance_m += calculate_distance(session_data[i], session_data[i + 1])
                    except Exception as dist_err:
                        logger.debug(f"Fehler bei Distanzberechnung für Overlay: {dist_err}")
                        continue
                
                # Dauer für diese Session aufsummieren
                try:
                    timestamps = [float(p['timestamp']) for p in session_data if 'timestamp' in p and p['timestamp'] is not None]
                    if timestamps:
                        start_time = min(timestamps)
                        end_time = max(timestamps)
                        total_map_duration_seconds += (end_time - start_time)
                except (ValueError, TypeError, KeyError) as time_err:
                    logger.warning(f"Konnte Dauer für Session in Overlay nicht ermitteln: {time_err}")

        # Werte formatieren
        map_distance_str = f"{total_map_distance_m / 1000:.2f} km" if total_map_distance_m > 0 else "N/A"
        map_duration_str = format_duration(total_map_duration_seconds) if total_map_duration_seconds > 0 else "N/A"
        # --- ENDE NEU: Berechnung ---
        ...
        # --- NEU: HTML-Overlay hinzufügen ---
        if flat_data_list:  # Nur hinzufügen, wenn Daten vorhanden sind
            # Definiere den HTML-Inhalt und das CSS-Styling
            html_content = f"""
             <div style="
                 position: fixed;
                 bottom: 10px;
                 ...
                 ">
                 <b>Gesamt (Karte):</b><br>
                 Strecke: {map_distance_str}<br>
                 Dauer: {map_duration_str}
             </div>
             """
            # Erstelle das HTML-Element und füge es zur Karte hinzu
            overlay_element = branca.element.Element(html_content)
            map_obj.get_root().html.add_child(overlay_element)
            logger.debug(f"HTML-Overlay für Gesamtwerte zu Karte {html_file} hinzugefügt.")
        # --- ENDE NEU: HTML-Overlay ---
            ...
            try:
                ...
                if temp_html_path.exists():
                    try:
                        ...
                        logger.warning(f"Konnte temporäre HTML {temp_html_path} nach Fehler nicht löschen: {rm_err}")
                return
            finally:
...
if __name__ == '__main__':
    ...
    multi_data_sats = [
        [  # Session 1 (älter)
            ...
            {'lat': 46.81175, 'lon': 7.13275, 'satellites': 5, 'timestamp': ts_now - 3600 * 2 + 10},  # Mehr Zeit
            {'lat': 46.8118, 'lon': 7.1328, 'satellites': 7, 'timestamp': ts_now - 3600 * 2 + 20},
        ],
        [  # Session 2 (neuer)
            {'lat': 46.81185, 'lon': 7.13285, 'satellites': 9, 'timestamp': ts_now - 60 * 5},  # Vor 5 min
            {'lat': 46.8119, 'lon': 7.1329, 'satellites': 11, 'timestamp': ts_now - 60 * 5 + 15},  # Mehr Zeit
            {'lat': 46.81195, 'lon': 7.13285, 'satellites': 4, 'timestamp': ts_now - 60 * 5 + 30},
        ]
    ]
    # Einzelne Session für heatmap_aktuell Test (etwas länger für Zeit-Heatmap)
    single_session_data = []
    start_single_ts = ts_now - 120  # Vor 2 Minuten starten
    for i in range(60):  # 60 Punkte über 120 Sekunden (alle 2 Sek)
        ts = start_single_ts + i * 2
    ...
    HEATMAP_CONFIG["test_heatmap_aktuell"] = {
        ...
        "show_start_end_markers": True,
        "path_colors": ["#3388ff"],
        "use_heatmap_with_time": False,  # Teste normale Heatmap + Pfad
        "use_satellite_weight": False,
        "visualize_quality_path": False,
    }
    HEATMAP_CONFIG["test_heatmap_aktuell_zeit"] = {  # Eigene Config für Zeit-Heatmap Test
        "output": "heatmaps/test_heatmap_aktuell_zeit.html",
        "png_output": "heatmaps/test_heatmap_aktuell_zeit.png",
        "generate_png": False,
        "radius": 8,
        "blur": 12,
        "path_weight": 2.0,
        ...
        "show_start_end_markers": True,
        "path_colors": ["#3388ff"],
        "use_heatmap_with_time": True,  # Hier Zeit-Heatmap testen
        "use_satellite_weight": False,
        "visualize_quality_path": False,
    }
    ...
    gen.create_heatmap(multi_data_sats, HEATMAP_CONFIG["test_quality_path_10"]["output"], draw_path=True,
    ...
    logger.info("Teste Heatmap für aktuelle Session (HTML) - Normal...")
    ...
    gen.create_heatmap(single_session_data, HEATMAP_CONFIG["test_heatmap_aktuell_zeit"]["output"], draw_path=True,
                       is_multi_session=False)
