        ...
        if visualize_quality_path:
            ...
            # --- Single-Session Fall (z.B. heatmap_aktuell) ---
                ...
                if session_data_single:
                    # ... (Daten sammeln und Distanz/Zeit berechnen - unverändert) ...
                    ...
                    if not heatmap_added:
                        if single_heat_data:
                            ...
                            # --- Änderung hier: Normale Heatmap auch in FeatureGroup verpacken ---
                            ...
                            heatmap_layers.append(heatmap_feature_group_single)  # Speichere die Gruppe
                            # --- Ende Änderung ---
            ...
            # ... (Bounds-Berechnung und Fallback - unverändert zur letzten Version) ...
            all_points_for_bounds = [[float(p['lat']), float(p['lon'])] for p in flat_data_list if
                                     'lat' in p and 'lon' in p]
...
if __name__ == '__main__':
    ...
    HEATMAP_CONFIG["test_heatmap_aktuell"] = {
        ...
        "show_start_end_markers": True,
        "path_colors": ["#3388ff"],
        "use_heatmap_with_time": False,  # HIER AKTIVIEREN -> Sollte False sein laut config.py
        "use_satellite_weight": False,
    ...
    logger.info(
        "Teste Heatmap für aktuelle Session (HTML) - Mit Zeitverlauf...")  # Log message is misleading if use_heatmap_with_time is False
    gen.create_heatmap(single_session_data, HEATMAP_CONFIG["test_heatmap_aktuell"]["output"], draw_path=True,
                       is_multi_session=False)

    logger.info("Heatmap-Generierungstests abgeschlossen.")  # heatmap_generator.py
import folium
import folium.plugins as plugins
import numpy as np
import logging
