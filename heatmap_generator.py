...
class HeatmapGenerator:
    ...
    def create_heatmap(self, data, html_file, draw_path, is_multi_session=False):
        ...
        total_map_duration_seconds = -1.0
        map_start_time_ts = None
        map_end_time_ts = None

        if flat_data_list:
            # Gesamtdistanz berechnen
            for i in range(len(flat_data_list) - 1):
                try:
                    total_map_distance_m += calculate_distance(flat_data_list[i], flat_data_list[i + 1])
            ...
            try:
                timestamps = [float(p['timestamp']) for p in flat_data_list if
                              'timestamp' in p and p['timestamp'] is not None]
                if timestamps:
                    map_start_time_ts = min(timestamps)
                    map_end_time_ts = max(timestamps)
                    total_map_duration_seconds = map_end_time_ts - map_start_time_ts
            except (ValueError, TypeError, KeyError) as time_err:
                logger.warning(
                    f"Konnte Gesamt-Start-/Endzeit für Kartenoverlay {html_file} nicht ermitteln: {time_err}")

        # Werte formatieren
        map_distance_str = f"{total_map_distance_m / 1000:.2f} km" if total_map_distance_m > 0 else "N/A"
        map_duration_str = format_duration(total_map_duration_seconds)
        # --- ENDE NEU: Berechnung ---
