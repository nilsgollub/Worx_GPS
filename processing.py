...
def apply_kalman_filter(gps_data, process_noise=0.05, measurement_noise=5.0, imu_yaw=None, dead_reckoning_enabled=True):
...
def process_gps_data(gps_data, config, geofences=None, latest_imu_data=None):
    ...
    data = apply_kalman_filter(
        ...
        imu_yaw=imu_yaw,
        dead_reckoning_enabled=config.get('dead_reckoning_enabled', True)
    )
