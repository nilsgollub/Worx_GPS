...
class WorxGps:
    ...
    def initial_heatmap_update(self):
        ...
        if processed_all:
            ...
            # Kumulierte Qualitäts- und WiFi-Karten (Multi-Session)
            self.update_single_map("quality_kumuliert", processed_all, draw_path=True, is_multi=True)
            self.update_single_map("wifi_kumuliert", processed_all, draw_path=True, is_multi=True)
    ...
    def handle_gps_data(self, csv_data):
        ...
        if csv_data != "-1":
            ...
            # Kumulierte Qualitäts- und WiFi-Karten (Multi-Session)
            self.update_single_map("quality_kumuliert", processed_sessions, draw_path=True, is_multi=True)
            self.update_single_map("wifi_kumuliert", processed_sessions, draw_path=True, is_multi=True)
