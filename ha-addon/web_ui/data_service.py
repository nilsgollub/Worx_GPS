...
class DataService:
    def __init__(self, project_root_path, heatmap_config, problem_config, geo_config_main, rec_config_main):
        ...
        logger.info("DataService initialisiert (Puffer leer).")
        self.reload_buffers()

    def reload_buffers(self):
        """Lädt alle internen Puffer frisch aus der Datenbank."""
        logger.info("DataService: Lade interne Puffer aus Datenbank neu...")
        ...
        # Falls jetzt gar keine Daten mehr da sind, ggf. Karten aufräumen?
        # (Wird beim nächsten generieren einfach leere Karten geben)
        logger.info(f"DataService: Buffers reloaded ({len(self._alle_maehvorgang_data)} Sessions gesamt).")


    def handle_gps_data(self, csv_data):
        """Verarbeitet empfangene GPS-Daten vom MQTT (wie Worx_GPS.handle_gps_data)."""
        if csv_data != "-1":
            # Zeitstempel-Validierung: Wenn wir neue Daten bekommen, prüfen wir auf große Lücken
            try:
                 parts = csv_data.split(',')
                 if len(parts) > 0:
                     try:
                         current_ts = float(parts[0])
                         if self._last_buffer_timestamp and (current_ts - self._last_buffer_timestamp > 3600):
                             logger.warning(f"Großer Zeitstempel-Sprung (>1h) im Puffer erkannt ({current_ts} vs {self._last_buffer_timestamp}). Starte neue Session automatisch.")
                             if self._gps_data_buffer:
                                 self.handle_gps_data("-1") # Verarbeite alten Puffer zuerst
                         self._last_buffer_timestamp = current_ts
                     except (ValueError, TypeError):
                         pass
            except Exception as e:
                logger.error(f"Fehler bei Puffer-Validierung: {e}")
        ...
        if not raw_gps_data:
            logger.warning("Keine GPS-Daten aus dem Puffer gelesen oder Puffer war leer.")
            return
        ...
        # Snapshot der aktiven Filter-Parameter mitspeichern
        filter_config_snapshot = {
            'method': config.POST_PROCESSING_CONFIG.get('method', 'kalman'),
            'kalman_measurement_noise': config.POST_PROCESSING_CONFIG.get('kalman_measurement_noise'),
            'kalman_process_noise': config.POST_PROCESSING_CONFIG.get('kalman_process_noise'),
            'hdop_threshold': config.POST_PROCESSING_CONFIG.get('hdop_threshold'),
            'max_speed_mps': config.POST_PROCESSING_CONFIG.get('max_speed_mps'),
            'dead_reckoning_enabled': config.POST_PROCESSING_CONFIG.get('dead_reckoning_enabled'),
            'outlier_detection_enable': config.POST_PROCESSING_CONFIG.get('outlier_detection', {}).get('enable'),
            'moving_average_window': config.POST_PROCESSING_CONFIG.get('moving_average_window'),
        }
        self.data_manager.save_gps_data(processed_data, filename, coverage=session_coverage, filter_config=filter_config_snapshot)
    ...
    def _generate_all_heatmaps(self, current_session_data=None):
        ...
        # 1. Aktuelle Session (heatmap_aktuell) - nur wenn Daten vorhanden
        if 'heatmap_aktuell' in self.heatmap_config and current_session_data:
            self._update_map('heatmap_aktuell', current_session_data, draw_path)
        
        # Multi-Session Daten vorbereiten (Liste von Listen)
        last_10_sessions = list(self._maehvorgang_data)  # Liste von Listen
        all_sessions = self._alle_maehvorgang_data  # Liste von Listen
        
        # 2. Letzte 10 Sessions (heatmap_10) - als Multi-Session
        if 'heatmap_10_maehvorgang' in self.heatmap_config and last_10_sessions:
            self._update_map('heatmap_10_maehvorgang', last_10_sessions, draw_path, is_multi=True)
        
        # 3. Alle Sessions kumuliert (heatmap_kumuliert) - als flache Heatmap, kein Pfad
        if 'heatmap_kumuliert' in self.heatmap_config and all_sessions:
            flat_all = flatten_data(all_sessions)
            if flat_all:
                self._update_map('heatmap_kumuliert', flat_all, draw_path=False, is_multi=False)
        
        # 4. GPS-Qualität (letzte 10 Sessions) - als Multi-Session
        if 'quality_path_10' in self.heatmap_config and last_10_sessions:
            self._update_map('quality_path_10', last_10_sessions, draw_path, is_multi=True)
        
        # 5. WiFi Signalstärke (letzte 10 Sessions) - als Multi-Session
        if 'wifi_heatmap' in self.heatmap_config and last_10_sessions:
            self._update_map('wifi_heatmap', last_10_sessions, draw_path, is_multi=True)
        
        # 6. GPS-Qualität kumuliert (alle Sessions) - als Multi-Session
        if 'quality_kumuliert' in self.heatmap_config and all_sessions:
            self._update_map('quality_kumuliert', all_sessions, draw_path, is_multi=True)
        
        # 7. WiFi Signalstärke kumuliert (alle Sessions) - als Multi-Session
        if 'wifi_kumuliert' in self.heatmap_config and all_sessions:
            self._update_map('wifi_kumuliert', all_sessions, draw_path, is_multi=True)
    ...
    def get_editable_config(self):
        ...
        # Nutze den expliziten Pfad zur .env Datei im Datenverzeichnis
        env_file_path = self.data_dir / ".env"
        ...
        def get_config_value(env_key, default_from_config, value_type=str):
            ...
            try:
                ...
                if value_type in [int, float] and value_to_convert is not None: return value_type(value_to_convert)
        ...
        return {
            ...
            'rec_test_mode': get_config_value('TEST_MODE', cfg_rec.get('test_mode', False), bool),
            'moving_average_window': get_config_value('MOVING_AVERAGE_WINDOW', config.POST_PROCESSING_CONFIG.get('moving_average_window', 5), int),
            'kalman_measurement_noise': get_config_value('KALMAN_MEASUREMENT_NOISE', config.POST_PROCESSING_CONFIG.get('kalman_measurement_noise', 5.0), float),
            'kalman_process_noise': get_config_value('KALMAN_PROCESS_NOISE', config.POST_PROCESSING_CONFIG.get('kalman_process_noise', 0.05), float),
            'hdop_threshold': get_config_value('HDOP_THRESHOLD', config.POST_PROCESSING_CONFIG.get('hdop_threshold', 2.5), float),
            'max_speed_mps': get_config_value('MAX_SPEED_MPS', config.POST_PROCESSING_CONFIG.get('max_speed_mps', 1.5), float),
            'outlier_detection': get_config_value('OUTLIER_DETECTION_ENABLE', config.POST_PROCESSING_CONFIG.get('outlier_detection', {}).get('enable', True), bool),
            'dead_reckoning': get_config_value('DEAD_RECKONING_ENABLED', config.POST_PROCESSING_CONFIG.get('dead_reckoning_enabled', True), bool),
            'gnss_mode': get_config_value('GPS_GNSS_MODE', config.GPS_CONFIG.get('gnss_mode', 'sbas'), str),
            'gps_serial_port': get_config_value('GPS_SERIAL_PORT', cfg_rec.get('serial_port', '/dev/ttyACM0'), str),
            'gps_baudrate': get_config_value('GPS_BAUDRATE', cfg_rec.get('baudrate', 9600), int),
            'debug_logging': get_config_value('DEBUG_LOGGING', cfg_rec.get('debug_logging', False), bool)
        }
