...
class TestWorxGps:
    ...
    def test_handle_gps_data_processing_success(self):
        ...
        self.worx_gps.alle_maehvorgang_data = [mock_parsed_data] # Mock DB return
        self.mock_data_manager_instance.load_all_mow_data.return_value = [mock_parsed_data]
        self.worx_gps.problemzonen_data.clear()

        # Patch process_gps_data so it just returns the input
        with patch('Worx_GPS.process_gps_data', return_value=mock_parsed_data) as mock_process:
            ...
            # data_manager methoden werden nicht mehr hier aufgerufen, sondern in data_service.py
            ...
            expected_data_last_10 = [mock_parsed_data] # Liste von Listen
            expected_flat_data = [point for sublist in [mock_parsed_data] for point in sublist]

            # --- Korrektur: problemzonen_heatmap wird nur aufgerufen, wenn Daten vorhanden sind ---
            expected_calls = [
                call(mock_parsed_data, MOCK_HEATMAP_CONFIG["heatmap_aktuell"]["output"], True, is_multi_session=False),
                call(expected_data_last_10, MOCK_HEATMAP_CONFIG["heatmap_10_maehvorgang"]["output"], True, is_multi_session=True),
                call(expected_data_last_10, MOCK_HEATMAP_CONFIG["quality_path_10"]["output"], True, is_multi_session=True),
                call(expected_data_last_10, MOCK_HEATMAP_CONFIG["wifi_heatmap"]["output"], True, is_multi_session=True),
                call(expected_flat_data, MOCK_HEATMAP_CONFIG["heatmap_kumuliert"]["output"], False, is_multi_session=False),
                call([mock_parsed_data], MOCK_HEATMAP_CONFIG["quality_kumuliert"]["output"], True, is_multi_session=True),
                call([mock_parsed_data], MOCK_HEATMAP_CONFIG["wifi_kumuliert"]["output"], True, is_multi_session=True),
            ]
            self.mock_heatmap_instance.create_heatmap.assert_has_calls(expected_calls, any_order=True)
    ...
    def test_handle_gps_data_processing_parse_fail(self, caplog):
        ...
        # publish_message ("error_gps") is no longer sent from Worx_GPS.py
    ...
    def test_handle_status_data_problem_success(self, caplog):  # caplog statt capsys
        ...
        # Die Liste wird durch read_problemzonen_data ersetzt
        assert self.worx_gps.problemzonen_data is expected_deque_after_append

        # remove_old_problemzonen is no longer called in Worx_GPS.py, but in data_manager.py indirectly via data_service (or via add_problemzone)
        # So we don't assert it here anymore. Same for save_problemzonen_data.

        self.mock_heatmap_instance.create_heatmap.assert_called_once_with(
            expected_deque_after_append,
            MOCK_HEATMAP_CONFIG["problemzonen_heatmap"]["output"],  # Nur der Pfad
            False,
            is_multi_session=False
        )

        # PNG-Erzeugung ist im Mock per Default False
        self.mock_heatmap_instance.save_html_as_png.assert_not_called()

        assert f"Empfangene Problemzonen-Daten: {csv_data}" in caplog.text
